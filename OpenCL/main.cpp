...
long unsigned int SIZE=1024; // column/row size.
...
int main(void)
{
  ...
  for(SIZE = 512; SIZE<=2048; SIZE = SIZE*2){ 
    std::cout<<std::endl<<std::endl<<"----------Simulation for Matrix Size = "<<SIZE<<"x"<<SIZE<<"-----------"<<std::endl<<std::endl;
    ...
    for(int j=0; j<SIZE; j++) {
		for(int i=0; i<SIZE; i++) {
			A[j*SIZE + i]     = (rand()%40)-20;
			B[j*SIZE + i]     = (rand()%40)-20;
			C[j*SIZE + i]     = 0;
			C_seq[j*SIZE + i] = 0;
		}
    }
	  ...
	  t1 = std::chrono::high_resolution_clock::now(); //Capture Time Start
    matrix_mul_sequence(A, B, C_seq);
    t2 = std::chrono::high_resolution_clock::now(); //Capture Time End
    ...
    // Device Types and Kernel Types
    std::string Device_types[2] = {"CPU","GPU"};
    std::string Kernel_types[3] = {"NORMAL","TILED_8","TILED_16"};
    
    // Loop through each device CPU and GPU
    for (int device=0;device<2;device++)   // loop 1 - CPU , loop 2 - GPU
    {
      std::cout<<endl<< "DEVICE TYPE: " << Device_types[device] <<endl;
    ...
    long unsigned int TILE_SIZE = 16;

    for( int kernel_id = 0; kernel_id <3; kernel_id ++) // Loop through all the Kernel types
    {
    
      std::cout<< "KERNEL TYPE: " << Kernel_types[kernel_id] <<endl;
      
      if(Kernel_types[kernel_id]== "TILED_8") { TILE_SIZE = 8;}
      else { TILE_SIZE = 16;}
    ...
    cl_kernel kernel;
    if(Kernel_types[kernel_id] == "NORMAL")
    ...
    //Verification Of Results of the computation
    bool validate = true;
    for(int j=0; j<SIZE; j++) {
		  for(int i=0; i<SIZE; i++) {
    ...
    free(source_str);
    //Freeing memory given to pony=ters
    A     = NULL;
    B     = NULL;
    C     = NULL;
    C_seq = NULL;
